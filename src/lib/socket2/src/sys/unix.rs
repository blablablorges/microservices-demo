...
// hyrbid patch: wasi-libc's fcntl returns EBADF for wasip2 sockets, so the
// F_GETFL above failed and every outbound connect() died with
// "tcp set_nonblocking error". std reaches the wasip2 API instead.
#[cfg(target_os = "wasi")]
pub(crate) fn set_nonblocking(fd: RawSocket, nonblocking: bool) -> io::Result<()> {
    use std::mem::ManuallyDrop;
    use std::os::fd::FromRawFd;
    let socket = ManuallyDrop::new(unsafe { std::net::TcpStream::from_raw_fd(fd) });
